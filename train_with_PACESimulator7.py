...
def main():
    ...
    fc_w = round_fcw(fc_w)
    np.savez('pace_mnist_int_bit2.npz', conv1w=conv1w, conv2w=conv2w, fc_w=fc_w)
    
    np.savez('pace_mnist_float_bit2.npz', conv1w=model_state['conv1.weight'].cpu(), conv2w=model_state['conv2.weight'].cpu(), fc_w=model_state['fc1.weight'].cpu())   
