...
class onn_conv2d(nn.Module):
    def __init__(self, input_channel, output_channel, filter_height=3, filter_width=3, stride=1, padding='SAME', add_bias=False,
        ...
        self.w_scale_factor = nn.Parameter(torch.Tensor(1))
        self.w_zero_point = nn.Parameter(torch.Tensor(1))

        self.out_scale_factor = nn.Parameter(torch.Tensor(1))
        self.out_zero_point = nn.Parameter(torch.Tensor(1))
    ...
    def reset_parameters(self) -> None:
        ...
        nn.init.constant_(self.w_scale_factor, 10)
        nn.init.constant_(self.w_zero_point, 0)

        nn.init.constant_(self.out_scale_factor, 0.01)
        nn.init.constant_(self.out_zero_point, 8)
    ...
    def forward(self, inp): # [b c h w]
        ...
        # w_scale_factor =(2**self.hardware.bits-1) / (self.weight.max()-self.weight.min()) 
        w_scale_factor =(2**self.hardware.bits-1) / (self.weight.max(0)[0]-self.weight.min(0)[0] + 1e-9) 
        ...
        # temp_out = (temp_out) / self.w_scale_factor - self.w_zero_point
        # temp_out = temp_out / (temp_out.max() - temp_out.min() + 1e-6) - (temp_out.max() + temp_out.min())/2
        # temp_out = (temp_out) * self.out_scale_factor - self.out_zero_point
...
class onn_fc(nn.Module):
    ...
    def forward(self, inp):
        ...
        # w_scale_factor =(2**self.hardware.bits-1) / (self.weight.max()-self.weight.min()) 
        w_scale_factor =(2**self.hardware.bits-1) / (self.weight.max(1)[0]-self.weight.min(1)[0] + 1e-9)         
        ...
        weight__ = self.onn_round( (self.weight + self.w_zero_point)* self.w_scale_factor) # 先对权值进行取整
        # weight__ = self.onn_round(self.weight * w_scale_factor )
        weight__ = self.onn_round(self.weight * w_scale_factor.unsqueeze(1) )
        weight__ = torch.clamp(weight__, -7, 7) 
